...
func (o *Orderbook) GetL2(depth int) []*types.L2Level {
	descending := o.AccountFlags.Is(AccountFlagBids)
	var levels []*types.L2Level
	o.Items(descending, func(node *SlabLeafNode) error {
		price := node.GetPrice()
		if len(levels) > 0 && levels[len(levels)-1].Price.Cmp(price) == 0 {
			levels[len(levels)-1].Size = utils.AddX(levels[len(levels)-1].Size, utils.BN(uint64(node.Quantity)))
		} else if len(levels) == depth {
			return errors.New("Depth reached")
		} else {
			levels = append(levels, &types.L2Level{
				Price: price,
				Size:  utils.BN(uint64(node.Quantity)),
			})
		}
		return nil
	})
